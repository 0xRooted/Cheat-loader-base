...
std::wstring utils_t::GetFullTempPath() {
...
bool utils_t::ReadFileToMemory(const std::wstring& file_path, std::vector<uint8_t>* out_buffer) {
...
bool utils_t::CreateFileFromMemory(const std::wstring& desired_file_path, const char* address, size_t size) {
...
uint64_t utils_t::GetKernelModuleAddress(const std::string& module_name) {
	void* buffer = nullptr;
	DWORD buffer_size = 0;
...
BOOLEAN utils_t::bDataCompare(const BYTE* pData, const BYTE* bMask, const char* szMask) {
...
uintptr_t utils_t::FindPattern(uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, char* szMask) {
...
PVOID utils_t::FindSection(char* sectionName, uintptr_t modulePtr, PULONG size) {
