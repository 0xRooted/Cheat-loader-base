...
std::wstring intel_driver::GetDriverPath() {
	std::wstring temp = utils_t::GetFullTempPath();
	if (temp.empty()) {
		return L"";
	}
...
HANDLE intel_driver::Load() {
	...
	if (!utils_t::CreateFileFromMemory(driver_path, reinterpret_cast<const char*>(intel_driver_resource::driver), sizeof(intel_driver_resource::driver)))
		return INVALID_HANDLE_VALUE;
	...
	ntoskrnlAddr = utils_t::GetKernelModuleAddress("ntoskrnl.exe");
	if (ntoskrnlAddr == 0) {
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}
...
uintptr_t intel_driver::FindPatternAtKernel(HANDLE device_handle, uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, char* szMask) {
	...
	auto result = utils_t::FindPattern((uintptr_t)sectionData, dwLen, bMask, szMask);
...
uintptr_t intel_driver::FindSectionAtKernel(HANDLE device_handle, char* sectionName, uintptr_t modulePtr, PULONG size) {
	...
	if (!ReadMemory(device_handle, modulePtr, headers, 0x1000))
		return 0;
	ULONG sectionSize = 0;
	uintptr_t section = (uintptr_t)utils_t::FindSection(sectionName, (uintptr_t)headers, &sectionSize);
...
bool intel_driver::ClearKernelHashBucketList(HANDLE device_handle) {
	uint64_t ci = utils_t::GetKernelModuleAddress("ci.dll");
	if (!ci)
		return false;
